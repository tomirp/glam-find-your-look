...
const BookingModal = ({ isOpen, onClose, muaData, services, initialServiceId }: BookingModalProps) => {
  ...
  // Fetch waktu yang tidak tersedia
  useEffect(() => {
  ...
  const handleConfirm = () => {
    ...
    const bookingData = {
      ...
      price: selectedService!.price_min, // Mengirim harga sebagai ANGKA
      ...
      <DialogContent className="max-w-md md:max-w-3xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-grow overflow-y-auto pr-4 -mr-4 space-y-6 py-4">
          <div className="space-y-3">
            <Label className="flex items-center gap-3 font-semibold text-lg"><span className="flex items-center justify-center h-8 w-8 rounded-full bg-primary text-primary-foreground">1</span>Pilih Layanan</Label>
            <Select onValueChange={setSelectedServiceId} value={selectedServiceId}>
              <SelectTrigger className="h-12 text-base"><SelectValue placeholder="Klik untuk memilih gaya makeup..." /></SelectTrigger>
              <SelectContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 items-start">
            <div className="space-y-3">
              <Label className="flex items-center gap-3 font-semibold text-lg"><span className="flex items-center justify-center h-8 w-8 rounded-full bg-primary text-primary-foreground">2</span>Pilih Tanggal</Label>
            ...
            <div className="space-y-3">
              <Label className="flex items-center gap-3 font-semibold text-lg"><span className="flex items-center justify-center h-8 w-8 rounded-full bg-primary text-primary-foreground">3</span>Pilih Waktu</Label>
              {!selectedDate ? (
                <div className="h-60 flex items-center justify-center text-center bg-accent/30 rounded-md"><p className="text-muted-foreground"><Info className="h-5 w-5 mx-auto mb-2" />Pilih tanggal terlebih dahulu.</p></div>
              ) : loadingTimes ? (
                ...
                <ToggleGroup type="single" variant="outline" onValueChange={setSelectedTime} value={selectedTime} className="grid grid-cols-4 gap-2">
                  {timeSlots.map(time => (
                    <ToggleGroupItem key={time} value={time} className="h-12 text-base" disabled={unavailableTimes.includes(time)}>{time}</ToggleGroupItem>
                  ))}
                </ToggleGroup>
              )}
            </div>
