import { useState, useEffect } from "react"; // <-- MODIFIKASI: Tambahkan useEffect
...
const Auth = () => {
  ...
  // <-- MODIFIKASI: Ambil user, role, dan loading dari context
  ...
  // <-- PENAMBAHAN: Hook untuk menangani redirect setelah login
  useEffect(() => {
    // Jalankan hanya jika proses loading auth selesai dan ada user yang login
    if (!authLoading && user) {
      if (role === 'mua') {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Navigasi tidak lagi di sini, akan ditangani oleh useEffect di atas.
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast({ title: "Berhasil!", description: "Akun berhasil dibuat. Silakan periksa email untuk verifikasi." });
      // Setelah daftar, Supabase akan otomatis login, dan useEffect di atas akan mengarahkan
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-secondary/20 flex items-center justify-center p-4">
        {/* ... (Sisa kode JSX tetap sama, tidak perlu diubah) ... */}
                        ...
                        <TabsContent value="register">
                            <form onSubmit={handleRegister} className="space-y-4">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <RadioGroup value={registerData.userType} onValueChange={(value: "customer" | "mua") => setRegisterData({ ...registerData, userType: value })} className="flex flex-row space-x-6">
