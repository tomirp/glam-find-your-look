...
// PERBAIKAN: Impor ikon Star
import { ArrowLeft, LogOut, MessageSquare, DollarSign, Star as StarIcon } from "lucide-react";
...
// PERBAIKAN: Impor tipe Review dan komponen ReviewsTab
import type { MUAProfile as MUAProfileType, UserProfile, Booking, Service, EditForm, Review } from "@/components/MUAProfile/types";
...
    const [services, setServices] = useState<Service[]>([]);
    // PERBAIKAN: Tambahkan state untuk ulasan
...
          setServices(servicesData || []);
          ...
          // PERBAIKAN: Ambil data ulasan untuk MUA
          const { data: reviewsData, error: reviewsError } = await supabase.from('reviews').select(`*, profiles!reviews_customer_id_fkey(full_name, avatar_url)`).eq('mua_profile_id', muaData.id);
...
                        <TabsTrigger value="layanan" className="data-[state=active]:bg-purple-100 data-[state=active]:text-purple-700 whitespace-nowrap px-4 py-2">Layanan & Portfolio</TabsTrigger>
                        {/* PERBAIKAN: Tambahkan tab "Ulasan" */}
                        <TabsTrigger value="ulasan" className="data-[state=active]:bg-purple-100 data-[state=active]:text-purple-700 whitespace-nowrap px-4 py-2">
                            <StarIcon className="h-4 w-4 mr-2" />Ulasan
                        </TabsTrigger>
...
            <TabsContent value="layanan"><ServicesPortfolioTab muaProfile={muaProfile} services={services} onPortfolioUpload={handlePortfolioUpload} onServiceAdded={fetchAllData} onProfileUpdate={fetchAllData} /></TabsContent>
                {/* PERBAIKAN: Tambahkan konten untuk tab "Ulasan" */}
                <TabsContent value="ulasan">
                    <ReviewsTab muaProfileId={muaProfile?.id || null} />
                </TabsContent>
